#!/usr/bin/env bash
# Record a samply CPU profile of `gitaur <args>` and emit a flat self/total
# time report. Profile is saved as profile.json.gz alongside a syms sidecar;
# load it later with `samply load profile.json.gz`.
#
# Usage: scripts/profile-refresh.sh [-o OUT] [-- <gitaur args>]
# Defaults to `gitaur -Sy` and ./profile.json.gz.
set -euo pipefail
...
for l in sys.stdin:
  ...
  if m.get("reason")=="compiler-artifact" and m.get("target",{}).get("name")=="gitaur" and m.get("executable"):
    print(m["executable"]); break')
[[ -x "$bin" ]] || { echo "could not locate gitaur binary" >&2; exit 1; }
